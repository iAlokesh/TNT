...
class CategoryPage{
    ...
    get heading(){
        return $("li>a[href='/category']") //css for heading the category page
    }

    get categoryButton(){
        return $('div>a[href="/category"]') //css for category button
    }

    get addNewButton(){
        return $("(//button[normalize-space()='ADD NEW'])[1]") //xpath for add new button
    }

    get categoryTextbox(){
        return $('input[name="categoryName"]') //css for category textbox
    }
    ...
    /*
    This method is to validate the heading of the category page 

    @param ValidationMessage - message which needs to be validated should be passed
    */
    async checkHeading(validationMessage){

        await expect(this.heading).toHaveTextContaining(validationMessage) //validating the validation message for check the heading of the category page

        report.addStep("Validate the heading of the category page",await browser.takeScreenshot(),"passed")
    }
    ...
    */
    async Category(){
        ...
        await this.categoryButton.click() //Clicking on the category button

        report.addStep("Click on the category button",await browser.takeScreenshot(),"passed")
    }
    ...
    */
    async CategoryBox(categoryName){
...
module.exports = new CategoryPage(); //This is for exporting the instance of CategoryPage class