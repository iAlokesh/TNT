...
class InactiveTokensTablePage {

    //Web Locators

    get heading(){
        return $("(//h3[normalize-space()='INACTIVE TOKENS'])[1]") //xpath for heading 
    }
    ...
    get retailerDropdown(){
        return $('#mui-component-select-id') //css for retailer dropdown
    }

    get retailerList(){
        return $$('ul[role="listbox"]>li') //css for retailer list
    }
    ...
    /*
    This method is to validate the heading of the inactive tokens table page

    @param ValidationMessage - message which needs to be validated should be passed
    */
    async checkHeading(validationMessage){

        await expect(this.heading).toHaveTextContaining(validationMessage) //validating the validation message for check the heading of the inactive tokens table page
        
        report.addStep("Validate the heading of the inactive tokens table page",await browser.takeScreenshot(),"passed")
    }
    ...
    */
    ...
    /*
    This method is for clicking on the retailer button and validate the values
    */
    async RetailerDropdown(){

        await this.retailerDropdown.click() //Clicking on the retailer dropdown

        await browser.pause(2000) //Pause browser

        report.addStep("Click on the retailer dropdown",await browser.takeScreenshot(),"passed")

        let List = await this.retailerList

        for(var i=0; i<List.length; i++){

            console.log("Retailers name : " + await List[i].getText())

            const dropdown = await List[i].getText()

            report.addStep('Retailers name : ' + `${dropdown}`);
        }
    ...
    */
    async ViewTxnButton(){
        ...
        await expect(browser).toHaveUrlContaining('transctions')
...
module.exports = new InactiveTokensTablePage() //This is for exporting the instance of InactiveTokensTablePage class