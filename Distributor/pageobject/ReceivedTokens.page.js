...
class RecivedTokensPage{

    //Web Locators

    get heading(){
        return $('h3[style="margin-left: 10px;"]') //css for recived tokens table
    }
    ...
    /*
    This method is to validate the heading of the received tokens table

    @param ValidationMessage - message which needs to be validated should be passed
    */
    async checkHeading(validationMessage){

        await expect(this.heading).toHaveTextContaining(validationMessage) //validating the validation message for check the heading of the received tokens table

        report.addStep("Validate the heading of the received tokens table",await browser.takeScreenshot(),"passed")
    }
...
module.exports = new RecivedTokensPage(); //This is for exporting the instance of RecivedTokensPage class